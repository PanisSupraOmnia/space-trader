...
public class PlunderDialog extends BaseDialog implements OnSingleClickListener {
    ...
    public void onSingleClick(View v) {
        getGameState().plunderFormHandleEvent(v.getId());
    }
    ...
    public void onResume() {
        ...
        getDialog().findViewById(R.id.dialog_plunder_dump).setOnClickListener(this);
    }

    @Override
    public void onRefreshDialog() {
        getGameState().drawPlunderForm();
    }

    @Override
    public void onClickPositiveButton() {
        getGameState().plunderFormHandleEvent(-1);
        dismiss();
    }

    @Override
    public int getHelpTextResId() {
        return R.string.help_plunder;
    }
}
